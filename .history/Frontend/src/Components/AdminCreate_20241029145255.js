...
const AdminCreate = () => {
  ...
  // Retrieve user info and check for access
  useEffect(() => {
    ...
    if (!userInfo) {
      ...
      const token = userInfo.token; // Ensure token is available for future requests

      // Optionally, you can add logic to restrict access to only admins
      if (userType !== "admin") {
