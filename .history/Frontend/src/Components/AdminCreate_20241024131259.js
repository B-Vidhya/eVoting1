...
const AdminCreate = () => {
  ...
  const [pic, setPic] = useState(null); // Changed to pic
  ...
  const postDetails = async (pics) => {
    ...
    if (pics.type === "image/jpeg" || pics.type === "image/png") {
      ...
      try {
        ...
        setPic(imageData.url); // Use setPic for saving image URL
      } catch (err) {
        console.log(err);
        toast({
  ...
  const handleSubmit = async (e) => {
    ...
    const eventData = {
      ...
      picture: pic, // Use pic for the image URL
      startNominationPhase,
      endNominationPhase,
      startVotingPhase,
      endVotingPhase,
      resultPhase,
      isActive, // Include isActive field in the payload
    };
