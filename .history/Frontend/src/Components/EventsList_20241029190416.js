...
const EventsList = () => {
  ...
  const token = userInfo?.token; // Get token if it exists

  // Fetch the events dynamically from the backend
  useEffect(() => {
    // Check if the token exists and redirect if not
    if (!token) {
      navigate("/home"); // Redirect to signup if not logged in
      return; // Exit early
    }
    ...
    const fetchEvents = async () => {
      try {
        ...
        if (error.response && error.response.status === 401) {
          navigate("/home"); // Redirect to signup if unauthorized
        }
      }
    };
      ...
      {userType === "admin" && (
        <Container className="mt-3">
          <Button
            ...
            className="mb-4 align-left" // Added class for custom alignment
          >
            Create Event
          </Button>
        </Container>
