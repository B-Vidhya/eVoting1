...
const NominationForm = () => {
  ...
  // If userInfo is null, redirect to home page
  useEffect(() => {
    if (!userInfo) {
      navigate("/home");
    }
  }, [userInfo, navigate]); // Ensure useEffect re-runs when userInfo changes

  const userType = userInfo?.type || ""; // Get user type (admin or user)
  const token = userInfo?.token; // Get the token, default to undefined if userInfo is null
  ...
  const handleSubmit = async (e) => {
    ...
    const jsonData = {
      ...
      image: pics, // Send the base64 image or URL if stored
      eventId: eventId,
    };
      ...
      const response = await axios.post(
        ...
        jsonData,
        {
          headers: {
            ...
            Authorization: `Bearer ${token}`, // Ensure token is used
          },
        }
      );
          ...
          <Col md={{ span: 8, offset: 2 }} className="form-container">
            ...
            <Form onSubmit={handleSubmit}>
              ...
              <Form.Group controlId="formImageUpload" className="image-upload">
                ...
                />
                {pics && (
                  <img src={pics} alt="Preview" className="image-preview" />
                )}
              </Form.Group>
