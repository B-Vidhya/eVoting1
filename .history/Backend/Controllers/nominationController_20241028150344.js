...
const getAcceptedNominations = async (req, res) => {
  try {
    const eventId = req.params.eventId.trim(); // Trim any whitespace
    console.log("Fetching accepted nominations for eventId:", eventId); // Log the eventId
    const nominations = await Nomination.find({
      eventId,
      status: "Accepted",
    });
    console.log("Accepted nominations found:", nominations); // Log found nominations
